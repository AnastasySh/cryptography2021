...
# шифрование
    ...
    for i in range(0, 8):
        ...
        # в S-блоках идет отчет от 0 до 15
        # создание массива "под ключ"
    key = [0]*64
    for i in range(0, 64):
    ...
    # 32 раунда
        for h in range(0, 32):
            ...
            # ксор левой и правой части и смена местами
                ...
                r[i] = l_zap
        # конец 32 раундов
    # сбор посылки обратно где числа от 0 до 71, чтобы гамма могла быть в рамках нашего алфавита
        for i in range(0, 4):
...
def main(message, file, mode):
    ...
    if file:
        ...
        message = f.read()
    if mode == 0:
        newstr, decrypto = Gamma(message)
        return (newstr, decrypto)
    if mode == 1:
        newstr, decrypto = prZamena(message)
        decrypto = decrypto.replace(' ','')
        return (newstr, decrypto)
    if mode == 2:
        newstr, decrypto = GammaOBR(message)
        return (newstr, decrypto)
    if mode == 3:
        newstr = imitovstavka(message)
        return (newstr)
    
    if file:

        if mode == 0:
            f = open('cryptGamma'+name, 'w')
            f.write(newstr)
        if mode == 1:
            f = open('cryptPrZamena'+name, 'w')
            f.write(newstr)
        if mode == 2:
            f = open('cryptGammaOBR'+name, 'w')
            f.write(newstr)
        if mode == 3:
            f = open('cryptImitovstavka'+name, 'w')
            f.write(newstr)
...
if __name__ == '__main__':
    ...
    name, mode,  file, message = '1', 3, True, '1000char.txt'
# 0 - гаммирование 1 - простой замены 2 - обратная гамма 3 - имитовставка
